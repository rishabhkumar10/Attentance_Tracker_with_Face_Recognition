...
class Leave:
    def __init__(self,root) :
     ...
     # first
        ...
        # Date
        Date1=Label(Details,text="Date leave From",font=("times new roman",12,"bold"),bg="white") 
        Date1.grid(row=5,column=0,padx=10,pady=5,sticky=W)

        date_entry=ttk.Entry(Details,textvariable=self.var_date2,width=15,font=("times new roman",12,"bold"))
        date_entry.grid(row=5,column=1,padx=10,pady=5,sticky=W)

        # Date
        Date=Label(Details,text="Date leave To",font=("times new roman",12,"bold"),bg="white") 
        Date.grid(row=5,column=2,padx=10,pady=5,sticky=W)

        date_entry=ttk.Entry(Details,textvariable=self.var_date,width=15,font=("times new roman",12,"bold"))
        date_entry.grid(row=5,column=3,padx=10,pady=5,sticky=W)

        # Reasone
        dep_lable=Label(Details,text="Reasone",font=("times new roman",12,"bold"),bg="white") 
        dep_lable.grid(row=6,column=0,padx=10,sticky=W)

        dep_combo=ttk.Combobox(Details,width=15,textvariable=self.var_reasone,font=("times new roman",10,"bold"),state="readonly")    
        dep_combo["values"]=("Select Reasone","Health Leave","Marriage Leave","Normal Leave")
        dep_combo.current(0)
        dep_combo.grid(row=6,column=1,padx=2,pady=6)


        # button frame
        bth_frame=Frame(Details,bd=2,relief=RIDGE,bg="white")
        bth_frame.place(x=0,y=205,width=650,height=35)
        # save
        save_btn=Button(bth_frame,text="Save",command=self.add_data,width=17,font=("times new roman",12,"bold"),bg="blue",fg="white")
        save_btn.grid(row=0,column=0)
        # update
        up_btn=Button(bth_frame,text="Update",width=17,command=self.update_data,font=("times new roman",12,"bold"),bg="blue",fg="white")
        ...
        # # # take photo sample
        # # take_btn=Button(bth_frame1,command=self.generate_data1,text="Take Photo Sample",width=35,font=("times new roman",12,"bold"),bg="blue",fg="white")
        ...
        table_frame.place(x=5,y=135,width=550,height=230)

        scroll_x=ttk.Scrollbar(table_frame,orient=HORIZONTAL)
        scroll_y=ttk.Scrollbar(table_frame,orient=VERTICAL)
        self.Employee_leave_Table=ttk.Treeview(table_frame,column=("Emp_Id","Emp_Name","Gen","DOB","Phone","Email","Dep","Add","photo"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)

        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)
        scroll_x.config(command=self.Employee_leave_Table.xview)
        scroll_y.config(command=self.Employee_leave_Table.yview)
        


        self.Employee_leave_Table.heading("Emp_Id",text="Employee_ID")
        self.Employee_leave_Table.heading("Emp_Name",text="Employee_NAME")
        self.Employee_leave_Table.heading("Gen",text="Gender")
        self.Employee_leave_Table.heading("DOB",text="Dtae OF Birth")
        self.Employee_leave_Table.heading("Phone",text="Phone")
        self.Employee_leave_Table.heading("Email",text="Email")
        self.Employee_leave_Table.heading("Dep",text="Department")
        self.Employee_leave_Table.heading("Add",text="Address")
        self.Employee_leave_Table.heading("photo",text="PhotoSampleStatus")
        self.Employee_leave_Table["show"]="headings"
        # width
        self.Employee_leave_Table.column("Emp_Id",width=100)
        self.Employee_leave_Table.column("Emp_Name",width=100)
        self.Employee_leave_Table.column("Gen",width=100)
        self.Employee_leave_Table.column("DOB",width=100)
        self.Employee_leave_Table.column("Phone",width=100)
        self.Employee_leave_Table.column("Email",width=100)
        self.Employee_leave_Table.column("Dep",width=100)
        self.Employee_leave_Table.column("Add",width=100)
        self.Employee_leave_Table.column("photo",width=150)

        self.Employee_leave_Table.pack(fill=BOTH,expand=1)
        self.Employee_leave_Table.bind("<ButtonRelease>",self.get_cursur)
        self.fetch_data()

        bth_frame=Frame(right_frame,bd=2,relief=RIDGE,bg="white")
        bth_frame.place(x=150,y=370,width=150,height=35)

        leave_btn=Button(bth_frame,text="Leave Details",command=self.leave,width=17,font=("times new roman",12,"bold"),bg="blue",fg="white")
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_emp_name.get()=="" or self.var_emp_id.get()=="":
            ...
            try:
                ...
                my_cursur.execute("insert into `leave` values(%s,%s,%s,%s,%s,%s,%s,%s)",(
                                self.var_emp_id.get(),
                                self.var_emp_name.get(),
                                self.var_gender.get(),
                                self.var_dep.get(),
                                self.var_dob.get(),
                                self.var_reasone.get(),
                                self.var_date2.get(),
                                self.var_date.get()
                             )) 
                ...
                messagebox.showinfo("Sucess","Employee Leave Details has been added Sucessfully",parent=self.root)
    ...
    def fetch_data(self):
        ...
        if len(data)!=0:
            self.Employee_leave_Table.delete(*self.Employee_leave_Table.get_children())
            for i in data:
                self.Employee_leave_Table.insert("",END,values=i)
            conn.commit()
        conn.close()
    
    # ==========get cursur=========
    def get_cursur(self,event=""):
        cursur_focus=self.Employee_leave_Table.focus()
        content=self.Employee_leave_Table.item(cursur_focus)
        data=content["values"]
        ...
        if self.var_dep.get()=="Select Department" or self.var_emp_name.get()=="" or self.var_emp_id.get()=="":
            ...
            try:
                ...
                if Update>0:
                    conn=mysql.connector.connect(host="localhost",username="root",password="96096",database="employee_attendance_system")
                    my_cursur=conn.cursor()
                    my_cursur.execute("update `leave` set Emp_Name=%s,Gender=%s,Dep=%s,Date=%s,Reasone=%s,DateLeaveFrom=%s,DateLeaveTo=%s where Emp_Id=%s",(
                    ...
                    if not Update:
                        return
                messagebox.showinfo("Sucess","Employee Leave details sucessfully",parent=self.root)
                conn.commit()
                self.fetch_data()
                self.reset_data()
                conn.close()
        ...
        if self.var_emp_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn=mysql.connector.connect(host="localhost",username="root",password="96096",database="employee_attendance_system")
                    my_cursur=conn.cursor()
                    sql="delete from `leave` where Emp_Id=%s"
                ...
                self.reset_data()
                conn.close()
                messagebox.showinfo("Delete","Sucessfully deleted Employee leave detail",parent=self.root)
    ...
    def reset_data(self):
        ...
        self.var_reasone.set("Select Reasone")
