...
class Attendd:
    def __init__(self, root):
        ...
        self.AttendanceReportTable=ttk.Treeview(table_frame,column=("Emp_Id","Emp_Name","Department","TotalNoOfAbsence","TotalNoOfLeave"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.AttendanceReportTable.heading("TotalNoOfAbsence",text="TotalNoOfAbsence")
        self.AttendanceReportTable.heading("TotalNoOfLeave",text="TotalNoOfLeave")
        ...
        self.AttendanceReportTable.column("TotalNoOfAbsence",width=100)
        self.AttendanceReportTable.column("TotalNoOfLeave",width=100)
...
# import mysql.connector
    ...
    # Connect to the leaveandabs database
        ...
        #     cursor = leaveandabs_db.cursor()
        #     cursor.execute("SELECT TotalNoOfAbsence, TotalNoOfLeave FROM leaveandabs")
        #     total_absence_count, total_leave_count = cursor.fetchone()
        #     cursor.close()

        # # Count the number of leaves and absences from the attendance database
        #     cursor = attendance_db.cursor()
        #     cursor.execute(
        #         "SELECT COUNT(*) FROM attendance WHERE Status LIKE '%%Leave%%'"
        #     )
        #     leave_count = cursor.fetchone()[0]
        #     cursor.execute(
        ...
        #     cursor = leaveandabs_db.cursor()
        #     cursor.execute(
        #         "UPDATE leaveandabs SET TotalNoOfAbsence = %s, TotalNoOfLeave = %s",
        #         (updated_absence_count, updated_leave_count)
        #     )
        #     leaveandabs_db.commit()
        #     cursor.close()
    ...
    # Close database connections
        ...
        #     cursor = leaveandabs_db.cursor()
        #     cursor.execute("SELECT Emp_Id, Emp_Name, Department, TotalNoOfAbsence, TotalNoOfLeave FROM leaveandabs")
        #     employees = cursor.fetchall()
        #     cursor.close()

        #     # Update leave and absence counts for each employee
        #     for emp_id, emp_name, department, total_absence_count, total_leave_count in employees:
        #         # Get the leave and absence counts from attendance table for today's date
